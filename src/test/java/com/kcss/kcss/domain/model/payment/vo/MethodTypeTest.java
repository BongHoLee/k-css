...
class MethodTypeTest {
    ...
    void 한글_영문_변환_테스트() {
        String send = "송금";
        String card = "카드";
        MethodType sendType = MethodType.kor(send);
        MethodType cardType = MethodType.kor(card);
    ...
    void 한글_영문_변환_예외_테스트() {
        String  ex = "예외";
        Assertions.assertThatThrownBy(() -> MethodType.kor(ex)).isInstanceOf(BusinessException.class);
    }
    ...
    void 영문_한글_변환_테스트() {
        String card = "CARD";
        Assertions.assertThat(MethodType.en(card)).isInstanceOf(MethodType.CARD.getClass());
    }
